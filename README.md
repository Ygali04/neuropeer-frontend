...
| GPU Compute      | Local NVIDIA A100/H100 or remote DataCrunch A100 via Verda spot instances     |
...
| `INFERENCE_BACKEND`  | No       | `local` (default) or `verda` for remote    |
| `VERDA_API_KEY`      | No       | Verda API key (remote GPU only)            |
